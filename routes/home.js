...
router.get("/", getUserStatus, async (req, res) => {
  ...
  playsUser = playsUser.filter(
    (play) => play.isPublic == true || play.creator == req.id
  );
  playsUser = playsUser.sort(
    (a, b) => b.createdAt.getTime() / 1000 - a.createdAt.getTime() / 1000
  );
...
router.get("/sort/likes", authAccess, getUserStatus, async (req, res) => {
  ...
  playsUser = playsUser.sort(
    (a, b) => b.usersLiked.length - a.usersLiked.length
  );
